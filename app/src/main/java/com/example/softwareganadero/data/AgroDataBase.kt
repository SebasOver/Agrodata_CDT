...
@Database(
    ...
    companion object {
        ...
        """.trimIndent())
                db.execSQL("CREATE INDEX IF NOT EXISTS index_weighings_created_at ON weighings(created_at)")
                db.execSQL("CREATE INDEX IF NOT EXISTS index_weighings_animal_number ON weighings(animal_number)")
            }
        }
        val MIGRATION_22_23 = object : Migration(22, 23) {
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL("""
            CREATE TABLE IF NOT EXISTS palpations(
              id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,
              animal_number TEXT NOT NULL,
              pregnancy_days INTEGER NOT NULL,
              observations TEXT,
              created_at INTEGER NOT NULL,
              created_at_text TEXT NOT NULL
            )
        """.trimIndent())
                db.execSQL("CREATE INDEX IF NOT EXISTS index_palpations_created_at ON palpations(created_at)")
